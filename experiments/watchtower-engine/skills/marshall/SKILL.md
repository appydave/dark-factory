...
```
- **Cleanup is HARNESS-driven, NOT self-action (David, 2026-06-06 — proven twice).** Self-action fails two ways: Marshall *forgetting* to close (stranded two build Swaggers), AND the Swagger *self-closing* (it has no permission to `tmux kill-window` its own window, and auto-mode blocks it — the self-close instruction left the Swagger idling, having tried and failed). ⟹ **do NOT rely on self-close.** The real mechanism: **Switchboard's staleness-detector emits `session.stale` → Marshall's Monitor (a privileged reaper, has tmux perms) reaps.** Until that loop is live, **Marshall MUST reap every Swagger after verifying its artifact** (step 4) — a hard, non-skippable step (failed twice; don't skip it again). See `cleanup-is-harness-driven-not-remembered`.
