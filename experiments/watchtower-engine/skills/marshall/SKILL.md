...
```
- **Cleanup is HARNESS-driven, NOT self-action (David, 2026-06-06 — proven twice).** Self-action fails two ways: Marshall *forgetting* to close, AND the Swagger *self-closing* (no tmux perm + auto-mode blocks it). ⟹ **do NOT rely on self-close.** **PROVEN MECHANISM (2026-06-07): the engine-state reaper.** On dispatch, write `registry/swagger-<slug>.json = {"queue_id": "<id>"}`. Run `bin/reaper.sh` as a standing **Monitor** in Marshall's session (the privileged reaper): it watches `reports/`, and once a handback lands + a grace period (default 60s) elapses, it `tmux kill-window`s the still-open window and deregisters — **no Marshall memory required.** Process-INDEPENDENT: the old Switchboard process-tree staleness-detector is a DEAD END (claude reparents to its daemon — `proof/reaper-livefire-finding.md`). This covers the **COMMON case (finished-but-open)**. The **STUCK case** (hung, no handback) still needs session-liveness (AngelEye/AppyRadar (was "AppyCtrl" — corrected 2026-06-10, see docs/appyradar.md)) — deferred. While the reaper Monitor is armed you don't hand-reap; if it isn't, reap manually after verifying (step 4). See `cleanup-is-harness-driven-not-remembered` + `docs/watchtower/reaper-brief.md`.
