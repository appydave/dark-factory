...
| 16 | Engine has no recovery for claimed-but-abandoned tickets: a dead/interrupted loop tick strands its entry in running/ forever | Timeout-requeue (running/ entry older than N min with no run record → back to queue/) or a reaper sweep | surfaced in real use 2026-06-01 | open |
