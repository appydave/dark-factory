...
| **AppyRadar** | **Read-only machine + fleet intelligence** — health, resource pressure (RAM/CPU/process), AI activity, maintenance debt across the 5-Mac network; exposed via Sentinel + MCP. **Does not kill.** (Supersedes the mis-named "AppyCtrl".) | AppySentinel (Pilot 1) | real SSH collector live (`scripts/audit.ts`, on M4 Mini); Sentinel rebuild in progress (`appyradar-sentinal`) |
