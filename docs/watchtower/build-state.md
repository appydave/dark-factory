...
| **Board v3 — Idea-staleness** | Flag concepts that have gone cold | ✅ 2026-06-06 (per-item "last touched" from `git blame` — capability-placement: git owns change-history; pure `attachStaleness()` + runtime `gitLineDates()` + age/stale UI; git-failsafe. Verified live: 48/51 items real git-dated, Floor untouched. `0 stale` is correct — register is all <14d old today). |
...
| **Auto-reaper (engine-state)** | Stranded Swagger windows close WITHOUT Marshall remembering | ✅ proven live 2026-06-07 — `bin/reaper.sh` (Marshall's Monitor) + dispatch registry (`registry/<window>.json`): handback lands + grace → kills the still-open window + deregisters, zero manual action. Process-independent (process-tree was a dead end). **Standing/production mode also proven 2026-06-07** (persistent Monitor, 60s grace, auto-closed `swagger-board-v4` during a real build — unattended). Covers the common case; stuck/no-handback → AngelEye/AppyRadar (was "AppyCtrl" — corrected 2026-06-10, see docs/appyradar.md) liveness (dispatched to AngelEye via Ralphy). `docs/watchtower/reaper-brief.md`. |
| **Board v4 — Floor↔Lanes bridge** | Promote a lane item → a floor job (planning→execution link) | ✅ 2026-06-07 — `POST /api/promote` (superseded by v5). Built by `swagger-board-v4` through the loop, auto-reaped by the standing reaper. |
| **Board v5 — Converge bridge (UX fix)** | Cluster-convergence replaces the confusing per-card promote | ✅ 2026-06-07 — multi-SELECT related concepts → ONE `POST /api/converge` brief-job; promote button + `/api/promote` removed; Floor/state/concepts/staleness untouched; verified live. |
| **Reaper hardened — keys off `done/`** | A skipped handback can't orphan a window | ✅ 2026-06-07 — `swagger-board-v5` finished but botched bookkeeping (skipped `reports/` handback + left a `running/` stray). Reaper now keys off `done/` (authoritative completion, lands before the handback; only successes). Open: a `running ∩ done → drop running` reconcile rule + harden run-next-workflow adherence (consumer-reliability gap). |
| **Retry-with-backoff** | Failed jobs re-queue with exponential backoff, then go terminal | ✅ 2026-06-07 — `bin/retry.sh` (Marshall's Monitor): a `failed/` ticket re-queues after `base·2^(attempt-1)` (default 30→60→120s) up to `RETRY_MAX` (3), else stays terminal in `failed/`. Symphony §8.4. Verified isolated (re-queue + give-up). Open: auto-dispatch of re-queued tickets (queue-Monitor, C3c). |
