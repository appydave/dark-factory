...
| **Board v4 — Floor↔Lanes bridge** | Promote a lane item → a floor job (planning→execution link) | ✅ 2026-06-07 — `POST /api/promote` writes a valid queue ticket + Lanes `promote ▶` button; built by `swagger-board-v4` through the loop, auto-reaped by the standing reaper. Floor/Lanes/`/api/state`/`/api/concepts` untouched. |
